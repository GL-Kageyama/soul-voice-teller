...
由 18 个技能构成——五个（`writer-persona` 映照写作者（人）的灵魂、声音与偏好，`premise` 构思，`plot-design` 设计，`fast-draft` 草稿，`voice-ledger` 存声），七个可选预备，以及文体 · 改稿 · 账本的六个——让「设计 → 草稿 → 改稿」仅凭写作技能即可成立。
...
- [18 个技能](docs/skills-zh.md) — 每个技能做什么（输入 → 输出）的参考
...
- `skills/{name}/SKILL.md` —— 18 个技能（en 正典）＋各语言的 `SKILL-ja.md` / `SKILL-zh.md`
