...
実行層            fast-draft ──→ ラフ散文（内省で逸脱を検出し計画を更新・発露と抑制の美学で届ける）
                  書く質 prose / scene-writer ／ 改稿 revise-for-reader / entertainment ／ 長期 series-bible
```
...
- `skills/{name}/SKILL.md` — 各スキルの正典源（15）。すべて独立して呼び出せる。`SKILL-ja.md` / `SKILL-zh.md` は各言語ミラー
