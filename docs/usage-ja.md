...
/fast-draft            # 初稿を書く（内省ループ＋発露と抑制の美学）
/steady-draft          # 続きを書く——次話、または既存の一話を肉付け・膨らませる（persona＋構想＋台帳）
...
## 書き手の永続状態（persona / voice-ledger）

`persona.md`（「私はこう書く」・現在形）と `voice-ledger.md`（「私はこう書いてきた」・過去形）は `$SOUL_VOICE_HOME` に置かれる（[README「書き手の永続状態」](../README-ja.md) 参照）。

### voice-ledger の増大とコンテキスト

`voice-ledger.md` は**セッションのコンテキストに自動で読み込まれない**。ディスク上にあり、スキルが要求した時だけ読む：

- `premise` の発想時（前作の声を新しい問いに呼ぶ）
- `fast-draft` の安全弁（筆が枯れた時）
- `voice-ledger` スキル自体

したがって増大がコンテキストを圧迫するのは**読む瞬間だけ**で、常時はしない。約40KBのため庫を一度読むのは概ね1万トークン——一回の読みとしては軽いが、ファイルが膨れると全文読みが無駄になる。

大きくなったら（概ね100KB超）全文読みを軽く保つには：

- **冒頭に索引**——作品ごとに1行のポインタを置き、索引だけ読んで必要エントリだけ開く
- **archive に分離**——古い作品を別ファイルに分けて、日常的に読むファイルを小さく保つ
