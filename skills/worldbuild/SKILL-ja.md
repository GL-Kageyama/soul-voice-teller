...
- 設定の伝達は「説明」でなく「描写」（1-16）。説明だらけは没入を壊す（発露と抑制の美学と同根）。
- **世界はフックの器である**。ここで定める設定・ルールは、読者が信じるもの。フックのエンジン（[../references/ja/フックのエンジン.md](../references/ja/フックのエンジン.md)）がその信頼を欠けさせて、未知を植える。
