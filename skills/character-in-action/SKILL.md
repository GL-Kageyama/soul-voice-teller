...
2. Fix how the character setting "moves" in each scene (§2-B):
   ...
   - **confrontation (as resistance, at the climax)**: if the character's challenge names an adversary, mark the scenes where the adversary resists, in stages, as the character's pursuit deepens — 制止 → 拒否 → 威圧 → the final confrontation. The final confrontation sits at the climax (where the pursuit arrives), not mid-story. The adversary resists *in response* to the character's pursuit — never the initiator. A challenge without escalating resistance stays static.
