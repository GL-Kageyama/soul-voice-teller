...
2. 文を文体の観点で検品する（§7）:
   ...
   - **読点の呼吸（読点は「流す」か「刻む」か）**: 並列は一息で流し、刻むのは**要の一打（反転・定型句）**だけ。普通の叙述は流す——「朝の光が、窓から差してた」のように。名詞句・動詞句・連語を原子に分解しない——「朝の、光が、窓から、差してた」「重くも、軽くも、ない」と切らない。一文で刻むのは**一か所**までに絞る。
   - **段落の切れ目（7-E）**: 反転・定型句の「要の一打」で切る。短文を詰め込んで余白を潰さない。
...
- **刻むのは要の一打だけ。** リズムは刻む量ではなく、**流す⇄刻むの配分**で作る。全編を刻むのは「息を止めっぱなし」で、逆に要の一打が効かなくなる。研ぐときはまず読点を削り、残すのは反転・定型句・人物の声だけにせよ。
- 修辞で埋めるのでなく、**余白と省略を効かせる**（7-25 簡潔体）。埋めたがる AI の癖を、persona の禁じ手・発露と抑制の美学で抑える。
