...
2b. Fix the character's **role** — what they *do* in the story (a person without a role does not move; the core alone leaves them static):
    ...
    - **challenge** (what they stand against): two parts — and the active verb belongs to the character, never the adversary:
        1. **adversary (named)**: who/what they stand against. Name them (an antagonist, not a vague "the organization").
        2. **the adversary's resistance (as response, in stages)**: how the adversary pushes back as the character's pursuit deepens — 制止 → 拒否 → 威圧 → the final confrontation. This resistance is the adversary's *response* to the character's pursuit, never the adversary's own initiative. The final confrontation sits at the climax (where the pursuit arrives), not mid-story.
        The character's pursuit itself (調べる・暴く・突きつける) belongs in the agency rule and the scene table, not here. Never write the adversary as the active subject ("the adversary pressures them") — that inverts the protagonist into a responder and collapses autonomy (measured -6/-24/-10 in V6).
