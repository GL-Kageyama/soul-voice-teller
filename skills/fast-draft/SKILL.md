---
name: fast-draft
description: 設計書（シーン表）と persona・ため庫から、場面ごとのラフ散文を書く草稿スキル。内省ループ（照合→逸脱→計画更新）で筆の乗りを検出し、抑制の美学（説明しない・名指ししない・総括しない・余白）で魂を運ぶ。執筆の本丸。plot-design の後。
...
三層モデルの**実行層**。魂の本丸。構想の個別性は魂を動かさない（E1/E4）、魂を動かすのは**書き方＝抑制**（E5）——その書き方をここで実装する。
...
## 出力仕様：抑制の美学（E5 で確定した魂のレバー）

書く前の指示ではなく、**書いた文を検品する基準**。各場面を書き終えるたび、次の4条に照らして検品する（[../references/抑制の美学.md](../references/抑制の美学.md)）:
...
混ざっていたら書き直す。「説明・名指し・総括」は**捏造された感傷（魂を壊す）と凡庸さ（個別性を壊す）の二重の害**だから。
