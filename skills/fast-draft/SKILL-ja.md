---
name: fast-draft
description: 設計書（シーン表）と persona・ため庫から、場面ごとのラフ散文を書く草稿スキル。内省ループ（照合→逸脱→計画更新）で筆の乗りを検出し、発露と抑制の美学（発露する・名指ししない・総括しない・余白・源を語らない）で魂を運ぶ。執筆の本丸。plot-design の後。
argument-hint: '（任意）design.md のパス。省略時は作業ディレクトリ内の design.md を探す'
---

# fast-draft —— 草稿（内省で逸脱を検出し、発露と抑制の美学で魂を運ぶ）
...
三層モデルの**実行層**。魂の本丸。構想の個別性は魂を動かさない。魂を動かすのは**書き方＝発露と抑制**。その書き方をここで実装する。
...
書く前の指示ではなく、**書いた文を検品する基準**。各場面を書き終えるたび、次の5条に照らして検品する（[../references/ja/発露と抑制の美学.md](../references/ja/発露と抑制の美学.md)）:
...
ground = **計画（シーン表）**。生成した文と計画を照合し、逸脱の有無を判定する。問いは [../references/ja/内省問い.md](../references/ja/内省問い.md) D章。
...
- **D-1 フローの検出**: この人物は計画通りか、勝手に動き始めたか。台詞・行動が計画の要求から出たか、人物自身から出たか。
- **D-2 逸脱の判定**（深める/壊す）: この逸脱は物語を深めるか壊すか。約束・伏線・結末への影響は。禁じ手（persona A-4）に反していないか。
- **D-3 計画の更新**: 逸脱を計画にどう写すか。旧計画への機械的回帰をしない。
- **D-4 安全弁（最重要）**: フローを**無理に作らない**。聞こえないのに聞こえたふりをしない。**聞こえないなら追従に戻る**——声は待つもので、押し出すものではない。追従に戻る前に、ため庫を聞く。
...
2. シーン表の**場面1から順に**書く。各場面:
   ...
   - 書いた文を**発露と抑制の美学5条**で検品し、破っていたら直す。
...
- `<作業ディレクトリ>/draft.md`（全場面を連結した草稿＝生成ビュー。場面ファイルから連結し直し、直接編集しない）
...
- **軸を場面ごとに小さく撃つ**。各場面は、その軸の三種のどれか一つは届けるべき（場面の弧。[../references/ja/6つの反応軸.md](../references/ja/6つの反応軸.md)）。どれも届けない場面は、読者が離れる場面。
- **フックを場面ごとに小さく撃つ**。各場面は、与える→欠けさせる（フックのエンジン。[../references/ja/フックのエンジン.md](../references/ja/フックのエンジン.md)）も行うべき——読者に何かを得させてから、それを欠けさせる。
