---
name: voice-ledger
description: 執筆者の魂の声を貯め、読み返すためのため庫（台帳）。内省（A/B/C/D問い）で聞こえた声・逸脱・筆の乗り・評価の響きを1件の書式で追記し、発想・停滞脱出・persona 更新に読み返す。単独でも、他の執筆スキルと併用でも使う。
...
- **貯める場合**: 声（6種）・その出所・文脈。argument-hint または対話で受け取る。
...
| 設計の声 | C問い・逸脱の記録 | 設計が約束から逸れて深まった場所、あるいは「この書き手のもの」と定まった択 |
| 草稿の声 | D問い・逸脱の記録 | 人物が計画を離れて動き始めた場所・その台詞 |
...
2. **貯める**: 声を受け取り、6種のどれかを判別し、1件の書式で `voice-ledger.md` に追記する。声は**原文のまま**、文脈（どの問いか）を必ず添える。
3. **読む**: 目的に応じてため庫を読み返し、該当する声を引用して返す。更新材料・発想の源・水源として提示する。

## 注意

- **ため庫自体は判定しない**——貯めて、読む。「何が声か」の選択は問いかけ型スキル（persona / premise / plot-design / fast-draft）が行う。
