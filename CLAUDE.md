...
- この反転は実証済み。E1/E4（n=2）で「構想の個別性は魂を動かさない（個別性だけを動かす）」、E5（n=2）で「魂を動かすのは書き方＝抑制」が確定した。
...
## 抑制の美学（魂のマスターレバー・E5 で確定）

- 魂を動かすのは構想の個別性ではなく、**書く仕方＝抑制**（E5: 魂 +21.5、個別性 +36.0）。
...
- 魂のレバーは E1/E4（構想＝個別性は魂を動かさない）と E5（書き方＝抑制が魂を動かす）で確定済み。
- 残る本丸は **SOURCE（照合＝persona）**。E2（persona 有無）で検証する。事前登録は [資料/小説執筆スキル構想/実証/実証_E2_事前登録.md](../資料/小説執筆スキル構想/実証/実証_E2_事前登録.md)。
